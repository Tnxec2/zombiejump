 ...
 * Most game objects should extend this class; lists of extensions can 
...
public class BaseActor extends Group {
    ...
    // keep track of how long the animation has been playing
    // and which image should be displayed
    // this value should be automaticaly updated
    ...
    // hit box, when not set in constructor is set to a boundaryRectangle
    ...
    public void setAnimation(Animation<TextureRegion> anim) {
        animation = anim;
        // get the first frame, to set width and height
    ...
    public Animation<TextureRegion> loadAnimationFromSheet(String fileName,
        int rows, int cols, float frameDuration, boolean loop) {
            ...
            int frameWidth = t.getWidth() / cols;
            int frameHeight = t.getHeight() / rows;
   ...
   // ----------------------------------------------
     ...
     *  @see #maxSpeed
     */
    public void applyPhsysic(float delta) {
        // apply acceleration
     ...
     *  @see #setAnimation
     */
    public Polygon setBoundaryRectangle() {
        float w = getWidth();
        float h = getHeight();
     ...
     */
    public Polygon setBoundaryPolygon(int numSides) {
        float w = getWidth();
        float h = getHeight();
    ...
    /**
     *  Returns bounding polygon for this BaseActor, adjusted by Actor's 
    ...
    public boolean overlaps(BaseActor other) {
        ...
        Polygon poly = this.getBoundaryPolygon();
        Polygon polyOther = other.getBoundaryPolygon();

        // initial test to improve perfomance
        if ( !poly.getBoundingRectangle().overlaps(polyOther.getBoundingRectangle()))
            return false;
        
        return Intersector.overlapConvexPolygons( poly, polyOther );
    }
...
/**
     ...
     * 
     * This calculation will not detect an exact distance from the original-boundry polygon
     * in every direction (for example, at the corners of a rectangular polygon), but it will
    ...
    {
        Polygon polyThis = this.getBoundaryPolygon();
        float scaleX = (this.getWidth() + 2 * distance) / this.getWidth();
        float scaleY = (this.getHeight() + 2 * distance) / this.getHeight();
        polyThis.setScale(scaleX, scaleY);
        
        Polygon polyOther = other.getBoundaryPolygon();
        
        // initial test to improve performance
        if ( !polyThis.getBoundingRectangle().overlaps(polyOther.getBoundingRectangle()) )
            return false;

        return Intersector.overlapConvexPolygons( polyThis, polyOther );
    }

    /*
    * set world bounds to the fix values
    */
    ...
    /*
    * set world bounds to the actor bounds
    */
    ...
    public void alignCamera() {
        ...
        cam.position.set(
                getX() + getOriginX(), getY() + getOriginY(), 0
            );
    ...
    /*
    * wraparound feature: when an object moves past the edge of the screen
    * it reappears on the opposite edge
    *
    * this method requires the worldbounds object to be set 
    */
